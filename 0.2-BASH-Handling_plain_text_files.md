...
``` 
 ...
 ```
   Lo anterior haría que en lugar de que la salida de `head`se imprima en el *stdout* se guarde en un archivo nuevo, en un archivo nuevo de texto plano, en este caso con nombre `products_head.csv`. 
   ...
   - imagina que el archivo original en cuestión `products.csv` estuviera compuesto por decenas de columnas (40, 50 o más) y cientos de miles -o millones- de filas. Con que archivo podrías trabajar más fácil en una aplicación de hojas de cálculo para análizar como está estructurada la información? 
      - a) `products.csv` que seguramente pesara por lo menos varios cientos de MB.
